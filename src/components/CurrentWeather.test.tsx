...
test('CurrentWeather', () => {
  ...
  expect(wrapper.find('.left-column div').html()).toBe(
    `<div class="hero">20 <span class="text-secondary">°</span></div>`
  );

  expect(wrapper.find('.right-column div')).toHaveLength(4);
  expect(wrapper.find('.right-column div').at(0).html()).toBe(
    '<div><span class="text-secondary">Precipitation:</span> precipitationStr</div>'
  );
  expect(wrapper.find('.right-column div').at(1).html()).toBe(
    '<div><span class="text-secondary">Humidity:</span> humidityStr</div>'
  );
  expect(wrapper.find('.right-column div').at(2).html()).toBe(
    '<div><span class="text-secondary">Wind:</span> windStr</div>'
  );
  expect(wrapper.find('.right-column div').at(3).html()).toBe(
    '<div><span class="text-secondary">Clouds:</span> cloudsStr</div>'
  );
});
